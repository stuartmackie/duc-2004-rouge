...
open my $ROUGEXML,">","rouge.xml";

print $ROUGEXML "<ROUGE-EVAL version=\"1.0\">\n\n";

for my $topic (@topics) {

  print $ROUGEXML "<EVAL ID=\"$topic\">\n";

  print $ROUGEXML "  <INPUT-FORMAT TYPE=\"SEE\"></INPUT-FORMAT>\n";

  # models
  print $ROUGEXML "  <MODEL-ROOT>models</MODEL-ROOT>\n";
  print $ROUGEXML "  <MODELS>\n";
  my $id=1;
  for my $goldstds ($duc2004{$topic}) {
    for my $goldstd (@$goldstds) {
      print $ROUGEXML "    <M ID=\"$id\">$goldstd.html</M>\n";
      $id++;
    }
  }
  print $ROUGEXML "  </MODELS>\n";
  ...
  # peers
  print $ROUGEXML "  <PEER-ROOT>peers</PEER-ROOT>\n";
  print $ROUGEXML "  <PEERS>\n";
  for my $system (@systems) {
    print $ROUGEXML "    <P ID=\"$system\">$topic.$system.html</P>\n";
  }
  print $ROUGEXML "  </PEERS>\n";
